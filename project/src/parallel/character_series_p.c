#include "parallel/character_series_p.h"
#include <stdio.h>
#include <unistd.h>
#include <sys/wait.h>
...
int multi_popular_series(const Sets *sets) {
    ...
    // выделение памяти для id процессов
    int status;
    int *pid = (int *)malloc(NUMBER_OF_PROCESSES * sizeof(int));
    if (pid == NULL) {
        return -1;
    }

    // создание нескольких процессов
    ssize_t step = get_size(sets) / NUMBER_OF_PROCESSES;
    for (ssize_t i = 0; i < NUMBER_OF_PROCESSES; ++i) {
        pid[i] = fork();
        if (pid[i] == -1) {
            free(pid);
            return -1;  // error create pid
        }
        else if (pid[i] == 0) {  // child
            ssize_t begin = i * NUMBER_OF_PROCESSES;
            ssize_t end = begin + step;
            ssize_t ind_popular = popular_series_range(sets, begin, end);
            if (ind_popular == -1) {
                free(pid);
                return -1;
            }
    ...
    // ожидаем завершение
    Sets *sets_proc = create_sets();
    for (ssize_t i = 0 ; i < NUMBER_OF_PROCESSES ; ++i) {
        while (waitpid(-1, &status, 0) > 0) {
            if (sets_push(sets_proc, get_series(sets, status)) != 0) {
                free(pid);
                return -1;
            }
        }
    ...
    ssize_t ind_popular = popular_series(sets_proc);
    free_sets(sets_proc);
